...
TEST(FacedetectTest, FacedetectTest)
{
  
  std::vector<std::string> goldenInputFiles = {"./golden_ifm.bin"};
  std::vector<std::string> goldenOutputFiles = {"./golden_ofm_0.bin", "./golden_ofm_1.bin"};
  std::vector<std::string> goldenOutputTensorNames = {"Postprocessor/scale_logits/aquant", "Squeeze/aquant"};
  ...
  {
    EXPECT_EQ(outputs[i]->get_tensor()->get_name(), goldenOutputTensorNames[i]);
