...
TEST(DWCTest, DWCTest)
{
  ...
  // Client Allocated, Owned, Memory
  std::vector<std::unique_ptr<vart::TensorBuffer>> inputs_owned = GraphEngine::create_inputs(graphRunner);
  std::vector<std::unique_ptr<vart::TensorBuffer>> outputs_owned = GraphEngine::create_outputs(graphRunner);
  ...
  std::vector<std::string> dumpOutputFiles = {"./dump_ofm.bin"};
