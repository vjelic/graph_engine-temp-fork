...
TEST(GraphRunnerTest, GraphRunnerBasic)
{
  // const xir::Subgraph *root = create_test_graph(0);
  const xir::Graph *graph = create_test_graph();

  // auto graphRunner = GraphEngine::create_graph_runner(root);
  auto attrs = xir::Attrs::create();
  auto graphRunner = GraphEngine::create_graph_runner(graph, attrs.get());
